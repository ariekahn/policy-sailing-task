...
export interface SailingInstructions {
    type: string,
    island_order: Array<string>;
    boat_order: Array<string>;
    instruction_set: string;
    allow_keyboard_response: boolean;
};
jsPsych.plugins['sailing-instructions'] = (function () {
    ...
    plugin.trial = async (display_element: Element, trial: SailingInstructions) => {
        ...
        const reward_ex_0 = await getElement('#reward_ex_0');
        const reward_ex_1 = await getElement('#reward_ex_1');
        const reward_ex_2 = await getElement('#reward_ex_2');
        const reward_ex_3 = await getElement('#reward_ex_3');
        const reward_ex_4 = await getElement('#reward_ex_4');
        const reward_ex_5 = await getElement('#reward_ex_5');
        const reward_ex_6 = await getElement('#reward_ex_6');
        const reward_ex_7 = await getElement('#reward_ex_7');
        const reward_ex_8 = await getElement('#reward_ex_8');
        const reward_ex_9 = await getElement('#reward_ex_9');
        const reward_ex_10 = await getElement('#reward_ex_10');
        const reward_ex_11 = await getElement('#reward_ex_11');
        const reward_ex_12 = await getElement('#reward_ex_12');
        const reward_ex_13 = await getElement('#reward_ex_13');
        const reward_ex_14 = await getElement('#reward_ex_14');
        const reward_ex_15 = await getElement('#reward_ex_15');
        const reward_ex_16 = await getElement('#reward_ex_16');
        const reward_ex_17 = await getElement('#reward_ex_17');
        const reward_ex_18 = await getElement('#reward_ex_18');
        const reward_ex_19 = await getElement('#reward_ex_19');
        const reward_ex_20 = await getElement('#reward_ex_20');
        const reward_ex_21 = await getElement('#reward_ex_21');
        const reward_ex_22 = await getElement('#reward_ex_22');
        const reward_ex_23 = await getElement('#reward_ex_23');

        for (let i = 0; i < 12; i += 1) {
            const x = 30 * (i % 4);
            const y = 50 + 30 * (Math.floor(i / 4));
            (await getElement(`#reward_ex_${i}`)).style.transform = `translate(${x}%, ${y}%)`;
            (await getElement(`#reward_ex_${i + 12}`)).style.transform = `translate(${x + 200}%, ${y}%)`;
            (await getElement(`#reward_ex_${i}`)).style.visibility = 'hidden';
            (await getElement(`#reward_ex_${i + 12}`)).style.visibility = 'hidden';
        }

        // Get HTML elements
        const ocean = await getElement('#ocean');
        const islands = await getElement('#islands');
        const background_zoom = await getElement('#background_zoom');
        const background_opacity = await getElement('#background_opacity');

        // const boat_0_left = await getElement('#boat_0_left');
        const boat_2_left = await getElement('#boat_2_left');
        const boat_1_right = await getElement('#boat_1_right');
        const boat_3_right = await getElement('#boat_3_right');
        ...
        const boat_dummy_right_container = await getElement('#boat_dummy_right_container');
        const boat_dummy_left_container = await getElement('#boat_dummy_left_container');
        ...
        const boat_dummy_bottom_container = await getElement('#boat_dummy_bottom_container');
        ...
        const no_merchant_link = await getElement('#no_merchant_link');
        const independent_turns = await getElement('#independent_turns');
        ...
        const instructions_back = await getElement('#jspsych-instructions-back') as HTMLButtonElement;
        const instructions_next = await getElement('#jspsych-instructions-next') as HTMLButtonElement;
        ...
        interface TrialData {
            ...
            island_response: null | number,
            boat_rt: null | number,
            boat_response: null | number,
            boat: null | number,
            reward: null | number,
        }
        ...
        function fade_in(elem: HTMLElement): void {
        ...
        function fade_out(elem: HTMLElement): void {
            elem.classList.add('fade_out');
            elem.classList.remove('fade_in');
            elem.classList.remove('partial_fade_in');
            elem.classList.remove('partial_fade_out');
        }
        function partial_fade_out(elem: HTMLElement): void {
            elem.classList.add('partial_fade_out');
            elem.classList.remove('partial_fade_in');
            elem.classList.remove('fade_out');
            elem.classList.remove('fade_in');
        }
        function partial_fade_in(elem: HTMLElement): void {
        ...
        const choose_boat_left = (island: number) => {
        ...
        const fade_in_island = (island: number) => {
            fade_in(dock_left);
            fade_in(dock_right);
        ...
        interface Scene {
            forward: Array<[Function, number, ...any[]]>;
            reverse: Array<[Function, number, ...any[]]>;
            text: string;
        }
        /* Scene
        forward: Functions to animate from prior scene
        reverse: animate from next scene
        text: text to add
        */
        let scenes_dict: { [key: string] : Array<Scene> } = {};
        scenes_dict['islands'] = [
            {
                forward: [
                    [show_ocean, 0],
        ...
        scenes_dict['boats'] = [
            ...
            },
            {
                ...
                reverse: [
                    [show, 0, reward],
                    [hide, 0, plus_one],
                    [() => { plus_one.style.transitionDuration = "0ms" }, 500],
                    [() => { plus_one.style.top = "51%" }, 0],
                ],
            ...
            },
            {
                forward: [
                    [hide, 500, reward],
                    [show, 500, plus_one],
                    [() => { plus_one.style.top = "40%" }, 500],
                    [() => { plus_one.style.transitionDuration = "1000ms" }, 500],
        ...
        scenes_dict['reward'] = [
            {
                ...
                reverse: [
                    ...
                    [() => { boat_2_left.style.left = '0%'; }, 0],
                    [() => { boat_3_right.style.left = '79.25%'; }, 0],
            ...
            },
            {
                forward: [
                    ...
                    [() => { boat_2_left.style.left = '26.41666667%'; }, 0],
                    [() => { boat_3_right.style.left = '52.83333333%'; }, 0],
        ...
        const scenes:Array<Scene> = scenes_dict[trial.instruction_set];
        let scene_idx = -1;
        ...
        function btnFwdListener(evt: any){
            remove_listeners();
            forward();
        }
        function btnBackListener(evt: any){
            remove_listeners();
            back();
        }
        const kbdListener = function (info: Response) {
            remove_listeners();
